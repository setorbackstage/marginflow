...
import {
  ...
  PRINT_JOB_STATUS_COLOR,
  PRINT_EVENT_LABEL,
  PRINT_TEMPLATE_TYPE_LABEL,
...
// ---------- QZ Tray status bar ----------
// Recebe o estado hoisted do PrintingPage para evitar instâncias duplicadas.

function PrintServiceStatus({ qz }: { qz: QZTrayState }) {
  return (
    <div className="rounded-lg border">
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1">
          ...
          {qz.error && <p className="mt-0.5 text-xs text-destructive">{qz.error}</p>}
        </div>
        {qz.status !== "connected" ? (
          <Button
            size="sm"
      ...
      {/* Instrução de instalação quando desconectado ou em erro */}
      {(qz.status === "disconnected" || qz.status === "error") && (
        <div className="border-t px-4 py-3 text-xs text-muted-foreground">
          O MarginFlow Print Service requer o{" "}
          <strong className="text-foreground">QZ Tray</strong> instalado e em execução no seu
          ...
          para baixá-lo, instale e abra antes de clicar em Conectar.
        </div>
      )}
    </div>
  )
...
function PrintersTab({ canEdit, qz }: { canEdit: boolean; qz: QZTrayState }) {
            ...
            <Card key={printer.id} className={!printer.isActive ? "opacity-60" : ""}>
              ...
              <CardContent className="space-y-1 text-xs text-muted-foreground">
                ...
                <p>
                  Interface:{" "}
                  <span className="text-foreground">
                    {PRINTER_INTERFACE_LABEL[printer.interface] ?? printer.interface}
                  </span>
                </p>
                {printer.model && (
                  <p>
                    Modelo: <span className="text-foreground">{printer.model}</span>
                  </p>
                )}
                {printer.address && (
                  <p>
                    Endereço:{" "}
                    <span className="font-mono text-foreground">{printer.address}</span>
                  </p>
                )}
                <p>
                  Status:{" "}
  ...
  return (
    <div className="space-y-4">
      ...
      {rules.data && rules.data.length > 0 ? (
              ...
              {rules.data.map((rule) => (
                <TableRow key={rule.id}>
                  <TableCell className="text-sm font-medium">
                    {PRINT_EVENT_LABEL[rule.event] ?? rule.event}
                  </TableCell>
        ...
        <EmptyState
          icon={ListChecks}
          title="Nenhuma regra de impressão"
          description="Configure regras para imprimir automaticamente quando eventos como 'Pedido confirmado' ou 'Pagamento recebido' ocorrerem."
        />
      )}
    </div>
  )
...
function JobsTab() {
  ...
  if (jobs.isError)
    return <ErrorState error={jobs.error} onRetry={() => jobs.refetch()} />
  ...
  return (
    <div className="space-y-4">
      ...
      {jobs.data && jobs.data.items.length > 0 ? (
        ...
        <EmptyState
          icon={Clock}
          title="Nenhum job de impressão"
          description="Os jobs de impressão aparecem aqui conforme os pedidos e eventos forem processados."
        />
      )}
    </div>
  )
    ...
    <div className="space-y-4">
      ...
      {templates.data && templates.data.length > 0 ? (
            ...
            <Card key={tpl.id} className={!tpl.isActive ? "opacity-60" : ""}>
              ...
              <CardContent className="text-xs text-muted-foreground">
                <p>
                  Tipo:{" "}
                  <span className="text-foreground">
                    {PRINT_TEMPLATE_TYPE_LABEL[tpl.type] ?? tpl.type}
                  </span>
                </p>
                <p>
                  Status:{" "}
                  <span className={tpl.isActive ? "text-green-600 dark:text-green-400" : ""}>
        ...
        <EmptyState
          ...
          description="Templates definem o layout dos documentos impressos (pedidos, comprovantes, etiquetas)."
        />
      )}
    </div>
  )
...
// ---------- Page ----------
// useQZTray é chamado UMA ÚNICA VEZ aqui e passado como prop para os filhos,
// garantindo estado único e consistente em toda a página.
    ...
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="printers">
        ...
        <TabsContent value="printers" className="mt-6">
          <PrintersTab canEdit={canEdit} qz={qz} />
        </TabsContent>
