...
export const customersApi = {
  list: (storeId: string, params: CustomerListParams): Promise<Page<CustomerListItem>> =>
    api.getPage<CustomerListItem>(`/stores/${storeId}/customers${qs({ page: params.page ?? 1, limit: 20, status: params.status, search: params.search })}`),
  get: (storeId: string, customerId: string) => api.get<CustomerDetail>(`/stores/${storeId}/customers/${customerId}`),
  create: (storeId: string, input: CreateCustomerInput) => api.post<CustomerDetail>(`/stores/${storeId}/customers`, input),
  ...
  listOrders: (storeId: string, customerId: string) =>
    api.getPage<CustomerOrderListItem>(`/stores/${storeId}/customers/${customerId}/orders?limit=10`),
}

export const addressesApi = {
  list: (storeId: string, customerId: string) => api.get<Address[]>(`/stores/${storeId}/customers/${customerId}/addresses`),
