...
| `print_receipt_on_confirm` | BOOLEAN | No | `false` | NOT NULL | When true, a customer receipt is automatically queued for printing when an order is confirmed. |
| `print_kitchen_ticket_on_confirm` | BOOLEAN | No | `false` | NOT NULL | When true, a kitchen ticket (comanda de cozinha) is automatically queued for printing when an order is confirmed. Independent of `print_receipt_on_confirm`. |
...
| `sku` | TEXT | Yes | NULL | — | Optional internal stock-keeping code. Used by the Inventory module (recipe/ficha técnica). |
| `ifood_external_code` | TEXT | Yes | NULL | — | External code used by iFood to identify this product in the iFood catalog. Required for product availability sync via `POST /integrations/IFOOD/sync`. |
...
- Unique index on `(store_id, external_id)` WHERE `external_id IS NOT NULL` — marketplace idempotency guard.
- GIN trigram index on `customer_name` using `pg_trgm` extension — enables fast ILIKE `%text%` search without full-table scan (migration `20260708080000_add_search_indexes`).
- Composite B-tree index on `(store_id, status, channel)` — orders dashboard filtered by status + channel (migration `20260708080000_add_search_indexes`).

| Column | Type | Nullable | Default | Constraints | Explanation |
|---|---|---|---|---|---|
| `id` | UUID | No | `gen_random_uuid()` | PK | Unique identifier. |
| `store_id` | UUID | No | — | NOT NULL, FK → stores.id RESTRICT | The store that received this order. |
| `customer_id` | UUID | Yes | NULL | FK → customers.id SET NULL | Null for anonymous walk-in orders and marketplace orders without a linked CRM record. When non-null, enables CRM and loyalty features. |
| `number` | INTEGER | No | — | NOT NULL, CHECK > 0 | Human-readable sequential number per store (e.g., 4821). Displayed as "#4821" in the UI. Generated by the service layer using `pg_advisory_xact_lock` inside a transaction. |
...
| `channel` | TEXT | No | — | NOT NULL, CHECK IN ('IN_STORE', 'PHONE', 'MARKETPLACE', 'WHATSAPP', 'KIOSK') | Sales channel through which the order entered the system. MARKETPLACE = order originated from iFood or another delivery platform. |
| `external_id` | TEXT | Yes | NULL | UNIQUE per `(store_id, external_id)` | The order ID assigned by the originating marketplace (e.g., iFood order UUID). Used for idempotent webhook ingestion — duplicate events with the same `external_id` are silently skipped. |
| `delivered_by` | TEXT | Yes | NULL | CHECK IN ('MERCHANT', 'IFOOD') when non-null | Who is responsible for the delivery logistics. `MERCHANT` = the store's own courier. `IFOOD` = iFood handles dispatch. Only set on MARKETPLACE orders. |
| `customer_name` | TEXT | Yes | NULL | — | Customer display name as received from the marketplace. Populated on webhook ingest for MARKETPLACE orders. For IN_STORE orders, the name comes from the linked `customers` record. |
| `customer_phone` | TEXT | Yes | NULL | — | Customer phone as received from the marketplace. Not linked to the `customers` table — this is the raw marketplace value. |
| `customer_document` | TEXT | Yes | NULL | — | Customer CPF or CNPJ as received from the marketplace (e.g., iFood's `customer.documentNumber`). |
| `table_number` | TEXT | Yes | NULL | — | Dine-in table identifier. Only meaningful when type = DINE_IN. |
| `delivery_address` | JSONB | Yes | NULL | — | Immutable address snapshot captured at order placement. Null for DINE_IN and TAKEAWAY orders. Format: `{ street, number, complement, neighborhood, city, state, postalCode, country, latitude, longitude, reference }`. The `reference` field (e.g., "portão azul") is populated from marketplace data. |
...
|---|---|---|---|---|---|
| `id` | UUID | No | `gen_random_uuid()` | PK | Unique identifier. |
| `platform` | TEXT | No | — | NOT NULL, UNIQUE, CHECK IN ('IFOOD', 'RAPPI', 'UBER_EATS') | The delivery marketplace platform. |
...
> **Security note:** The `access_token` is stored in plaintext because it is not a secret that can be derived from user input — it is obtained from the marketplace OAuth server using the app's `client_id`/`client_secret` (which live in environment variables, never in the database). The token grants access only to polling events and managing linked merchants, not to billing or account data.
...
**Purpose**
Records the connection between a Store and a delivery marketplace merchant account. Each row represents one store's presence on one platform (e.g., "Store X is connected to iFood with merchant ID abc123"). A store can have one integration per platform.

This is the operational connection record. The OAuth token for making API calls lives in `marketplace_app_configs`; the merchant ID needed to scope API calls to this store lives here.
...
**Unique Constraints**
- `(store_id, platform)` — a store can connect to each platform at most once.

**Foreign Keys**
- `store_id → stores.id` (CASCADE on delete — integration is removed when the store is deleted)

**Indexes**
- Primary key on `id` (automatic).
- Unique index on `(store_id, platform)`.
- Index on `(platform, status)` — `pollAllMarketplaceStores()` fetches all ACTIVE integrations for a platform in a single query.

| Column | Type | Nullable | Default | Constraints | Explanation |
|---|---|---|---|---|---|
| `id` | UUID | No | `gen_random_uuid()` | PK | Unique identifier. |
| `store_id` | UUID | No | — | NOT NULL, FK → stores.id CASCADE | The store that owns this integration. |
| `platform` | TEXT | No | — | NOT NULL, CHECK IN ('IFOOD', 'RAPPI', 'UBER_EATS') | The delivery marketplace platform. |
| `merchant_id` | TEXT | No | — | NOT NULL | The store's identifier in the marketplace (e.g., iFood merchant UUID). Used as the API scope for all marketplace calls. |
| `status` | TEXT | No | `'ACTIVE'` | NOT NULL, CHECK IN ('ACTIVE', 'INACTIVE', 'ERROR') | ACTIVE = polling and order ingestion are running. INACTIVE = manually disconnected. ERROR = last sync failed with an unrecoverable error. |
| `is_paused` | BOOLEAN | No | `false` | NOT NULL | When true, the store is "closed" on the marketplace (e.g., iFood shows "Loja fechada"). This is separate from `status` — a paused store is still connected, just not accepting new orders. |
| `last_sync_at` | TIMESTAMPTZ | Yes | NULL | — | UTC timestamp of the last successful event poll. Null if the integration was never successfully polled. Used to detect stale integrations. |
| `error_message` | TEXT | Yes | NULL | — | Last error message from the marketplace API. Set when `status = 'ERROR'`. Cleared on next successful sync. |
| `created_at` | TIMESTAMPTZ | No | `now()` | NOT NULL | UTC timestamp when the integration was connected. |
| `updated_at` | TIMESTAMPTZ | No | `now()` | NOT NULL | UTC timestamp of last status change. |

> **Design note — `is_paused` vs `status`:** `status` tracks the health of the integration (is the connection working?). `is_paused` tracks the operational intent (should the store accept orders right now?). A paused, healthy store has `status = 'ACTIVE'` and `is_paused = true`. An unhealthy store has `status = 'ERROR'` regardless of `is_paused`.
